    ...
    <div>
      ...
      <div className="mb-6 p-4 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl border border-indigo-100">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-xs font-bold text-gray-700 uppercase tracking-wider">Trending Now</h3>
        </div>
        <div className="space-y-2">
          {[wantToActions[0], wantToActions[3], useActions[1]].map((action, idx) => {
            const Icon = action.icon;
            return (
              <button
                key={idx}
              ...
              >
                ...
                <span className="text-sm text-gray-800 font-medium truncate text-left">
                  {action.label}
                </span>
              </button>
            );
